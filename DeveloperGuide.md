# Morphe PDF Developer Guide

## 1. Introduction

Morphe PDF is a robust, locally hosted, web-based PDF manipulation tool. **Stirling 2.0** represents a complete frontend rewrite with a modern React SPA (Single Page Application).
...
4. Lombok Setup
Morphe PDF uses Lombok to reduce boilerplate code. Some IDEs, like Eclipse, don't support Lombok out of the box. To set up Lombok in your development environment:
Visit the [Lombok website](https://projectlombok.org/setup/) for installation instructions specific to your IDE.

5. Add environment variable
For local testing, you should generally be testing the full 'Security' version of Morphe PDF. To do this, you must add the environment flag DISABLE_ADDITIONAL_FEATURES=false to your system and/or IDE build/run step.
...
Morphe PDF can be packaged as a cross-platform desktop application using Tauri with PDF file association support and bundled JRE.
...
Morphe PDF offers several Docker versions:
...
Morphe PDF provides several example Docker Compose files in the `exampleYmlFiles` directory, such as:
...
services:
  stirling-pdf:
    ...
    environment:
      ...
      UI_APPNAME: Morphe PDF
      UI_HOMEDESCRIPTION: Demo site for Morphe PDF Latest with Security
      UI_APPNAMENAVBAR: Morphe PDF Latest
...
Morphe PDF uses different Docker images for various configurations. The build process is controlled by environment variables and uses specific Dockerfile variants. Here's how to build the Docker images:
...
Morphe PDF also provides a `test.sh` script in the root directory for Docker integration tests. This script builds all versions of Morphe PDF, checks that each version works, and runs Cucumber tests. It's recommended to run this script before submitting a final pull request.
...
For quick iterations and development of Java backend, JavaScript, and UI components, you can run and test Morphe PDF locally without Docker. This approach allows you to work on and verify changes to:
...
Morphe PDF can be customized through environment variables or a `settings.yml` file. Key customization options include:
...
For managing language translations that affect multiple files, Morphe PDF provides a helper script:
...
## Adding New Translations to Existing Language Files in Morphe PDF

When adding a new feature or modifying existing ones in Morphe PDF, you'll need to add new translation entries to the existing language files. Here's a step-by-step guide:
