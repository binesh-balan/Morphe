...
def collect_known_signatures(signatures_dir: Path) -> dict[str, dict]:
    ...
    for json_file in signatures_dir.rglob("*.json"):
        ...
        except Exception as exc:  # noqa: BLE001 - logs/handles, deliberately broad in a batch tool
            print(f"[WARN] Skipping unreadable signature file {json_file}: {exc}")
            continue
        pdf = payload.get("pdf")
...
def write_report(report_path: Path, fonts_by_signature: dict[str, dict]) -> None:
    ...
    report = {
        # Local report timestamp; "Z" here is a display convention, not a UTC guarantee.
        "generatedAt": dt.datetime.utcnow().isoformat(timespec="seconds") + "Z",  # noqa: DTZ003
