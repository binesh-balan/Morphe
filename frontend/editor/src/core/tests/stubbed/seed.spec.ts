...
/**
 * Seed test for Morphe PDF E2E tests.
...
test.describe("Morphe PDF seed", () => {
  test("seed - app loads", async ({ page }) => {
    // Navigate to the Morphe PDF frontend
    ...
    await expect(
      page
        .locator(
          '.h-screen, .mobile-layout, [data-testid="dashboard"], img[alt*="Morphe"]',
        )
        .first(),
    ).toBeVisible({ timeout: 15000 });

    // Verify the title contains Morphe PDF
    await expect(page).toHaveTitle(/Morphe/i);
  });
});
